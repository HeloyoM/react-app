...
function CardsContainer() {
    ...
    vacationsArray.forEach(function(vac) {
        return vac.follow = false;
    }); //for remove vacations by admin:

    var vacationSelectedByAdmin = react_redux_1.useSelector(function(state) {
        return state.vacationSelectedByAdmin;
    });
    removeVacation(vacationSelectedByAdmin, vacationsArray); //follow btn was clicked by user:

    var newItemOnTrack = react_redux_1.useSelector(function(state) {
        return state.newItemOnTrack;
    });
    var userName = react_redux_1.useSelector(function(state) {
        return state.userName;
    });
    addRowToFollwingVacationTable(newItemOnTrack, userName, vacationsArray); //for update vacation by admin:

    var vacationToUpdate = react_redux_1.useSelector(function(state) {
        return state.vacationToUpdate;
    });
    var vacationChoosen = react_redux_1.useSelector(function(state) {
        return state.vacationChoosen;
    });
    updateVacationDetails(vacationToUpdate, vacationChoosen, vacationsArray);
    var myVacations = react_redux_1.useSelector(function(state) {
        return state.myVacations;
    });

    if (myVacations.length !== 0) {
        var filterArray_1 = [];
        myVacations.forEach(function(item) {
            return filterArray_1.push(vacationsArray[item]);
        });
    ...
    }, userType === "admin" && react_1["default"].createElement("div", {
        className: "AdminBtns"
    }, react_1["default"].createElement("h3", null, react_1["default"].createElement("strong", null, "Hi admin, wellcome back")), react_1["default"].createElement("button", {
        className: "AddVacationBtn",
        onClick: onAddBtnClicked
    }, "Add Vacation"), react_1["default"].createElement("button", {
        className: "ReportBtn",
        onClick: onReportClicked
    }, "Reports graph")), vacationsArray.map(function(deal, index) {
        return react_1["default"].createElement(Card_1["default"], {
            key: index,
            vacationId: index,
            img: deal.img,
            title: deal.title,
            dscription: deal.dscription,
            date: deal.date,
            price: deal.price,
...
function removeVacation(vacationName, vacationsArray) {
    ...
    var vacationObject = vacationsArray.find(function(vac) {
        return vac.title === vacationName;
    });
    ...
    if (dbIndex === -1 && arrayIndex === -1) {
        ...
        axios_1["default"]["delete"]("http://localhost:3002/vacations/" + dbIndex).then(function(res) {
...
function addRowToFollwingVacationTable(newItemOnTrack, userName, vacationsArray) {
    ...
    counter++;
    var vacationObject = vacationsArray.find(function(vac) {
        return vac.title === newItemOnTrack;
    });
    ...
    if (vacationObject === undefined || counter === 1) {
        //i add'd here a counter becuse the useEffect function rendering the component twice and then add'd tow rows to the table; need to learn more
        return; //<--bug!
    }
    ...
    axios_1["default"].post("http://localhost:3002/follow", newFollowPacket).then(function(res) {
...
function updateVacationDetails(vacationToUpdate, vacationChoosen, vacationsArray) {
    ...
    var vacationObject = vacationsArray.find(function(vac) {
        return vac.title === vacationChoosen;
    });
    var vacationId = vacationObject.vacationId;
